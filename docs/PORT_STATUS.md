...
|---|---|---|---|
| YCSB A-F | `ycsb-worktable`, `ycsb-sqlite`; D diagnostic: `campaigns/ycsb-index-backends` | Paired WorkTable and SQLite `:memory:` runners use the same pre-generated A-F templates, ten-field approximately 1 KiB rows, Zipf/latest/scan distributions, latency sampling, and thread counts; Workload D uses an execution-time contiguous acknowledged-insert frontier; concurrent D is clean with stabilized WorkTablesIndex, memory-only Congee/Arctic, or gated upstream IndexSet miss confirmation | Integrate the selected PR branches upstream, durable-store adapters, controlled ARM confirmation, and official campaign results |
