...
|---|---|---|---|
| YCSB A-F | `ycsb-worktable`, `ycsb-sqlite` | Paired WorkTable and SQLite `:memory:` runners use the same pre-generated A-F templates, ten-field approximately 1 KiB rows, Zipf/latest/scan distributions, latency sampling, and thread counts; Workload D uses an execution-time contiguous acknowledged-insert frontier; per-operation errors and SQLite lock retries are counted | Durable-store adapters, WorkTable's rare transient concurrent-insert point miss, and official campaign results |
