...
| Resident provenance point lookup | `src/moe_resident.rs`, `src/bin/moe-resident-index-ab.rs` | A measured resident-IR shape: 1,528 unique `(source, ordinal)` keys, five-field owned rows, and one million deterministic successful lookups. Compares linear `Vec`, `Vec+BTreeMap`, `Vec+Arctic`, and generated WorkTable with Arctic, WTI, and Congee. Published WorkTable `1.0.0-beta.17`, `persist:false`: Arctic is the fastest full-table backend at about 26 ns/query; the stripped `Vec+Arctic` control is about 5.3 ns/query. | Runnable; equality checked |
| Resident-memory overhead | `src/moe_resident.rs`, `src/bin/moe-resident-memory-ab.rs` | Isolated-process allocation census for the same 1,528 rows and checksum. Published WorkTable `1.0.0-beta.17`, `persist:false`: WorkTable+Arctic retains 396,548 B versus 105,824 B for `Vec+Arctic` (3.75x), and leaves 73,004 B after table drop. WTI and Congee full-table controls distinguish table-layer overhead from the index backend. | Runnable; beta.17 regression reproducer |
