...
| P0 | db_bench core | fillseq, fillrandom, overwrite, readrandom, readseq, readreverse, seekrandom, delete; fixed key/value widths | Recognizable embedded KV envelope; report that WorkTable is typed rows, not an LSM KV store. |
...
| P0 | SQLite speedtest1 core | transactional bulk insert, indexed point/range reads, ordered scans, updates, deletes, text and integer keys | Public-domain, desktop-relevant operations. Do not claim the unsupported SQL query groups. |
...
| P1 | LinkBench | link add/delete/update/count/get, node get/update, ID and time range queries with social-graph skew | Many-to-many fan-out, non-unique indexes, hot nodes, and mixed mutation. Apache-2.0. |
